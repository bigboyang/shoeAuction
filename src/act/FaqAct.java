...
public class FaqAct implements Action{
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		else if (!isEmpty(status) && status.equals("a")) where += " and bf_cata = '구매' ";
		else if (!isEmpty(status) && status.equals("b")) where += " and bf_cata = '입찰' ";
		else if (!isEmpty(status) && status.equals("c")) where += " and bf_cata = '배송' ";
		else if (!isEmpty(status) && status.equals("d")) where += " and bf_cata = '기타' ";
		...
		if (ord == null || ord.equals(""))	ord = "idxa";	// 등록 역순으로 정렬이 기본값
