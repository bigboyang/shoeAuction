...
} else if (pageInfo.getXline().equals("quaility")) {
   labels = "'s', 'a', 'b'";
   int s = 0;   int a = 0;   int b = 0;
   
   if (saleOption != null) {
      for (StatsInfo si : saleOption) {      
         if (si.getX_saleOption().equals("s"))       s += si.getY_saleOption();
         else if (si.getX_saleOption().equals("a"))   a += si.getY_saleOption();
         else if (si.getX_saleOption().equals("b"))   b += si.getY_saleOption();
      }
   }
...
if (pageInfo.getYline().equals("count")) {
	label = "판매개수";
   if (pageInfo.getXline().equals("brand")) text = "'브랜드별 매출건수'";
   else if (pageInfo.getXline().equals("size")) text = "'사이즈별 매출건수'";
   else text = "'상품등급별 매출건수'";
   
} else {
	label = "판매액";
   if (pageInfo.getXline().equals("brand")) text = "'브랜드별 매출액'";
   else if (pageInfo.getXline().equals("size")) text = "'사이즈별 매출액'";
   else text = "'상품등급별 매출액'";
}
...
<div id="wrapper">
   ...
   <td>
      <select name="xline">
         ...
         <option value="quaility" <% if (pageInfo.getXline().equals("grade")) { %> selected="selected"<% } %>>상품등급</option>
      </select>
   </td>
   <td>
