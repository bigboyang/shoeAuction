...
<p style="width:800px;" align="right">정렬조건 :  
      ...
      <option value="mp_buyauc.auc<%=args %>&ord=startd" <% if (ord.equals("startd")) {%> selected = "selected" <%} %>>최신순</option>
      <option value="mp_buyauc.auc<%=args %>&ord=starta" <% if (ord.equals("starta")) {%> selected = "selected" <%} %>>오래된순</option>
   </select>
</p>
...
if (pdtList != null && pdtList.size() > 0) {	
	...
	if (cpage == 1) {	
		...
		out.print("<a href='mp_buyauc.auc?cpage=1" + args + "'>");
		out.println("[&lt;&lt;]</a>&nbsp;");
		out.print("<a href='mp_buyauc.auc?cpage=" + (cpage - 1) + args + "'>");
		out.println("[&lt;]</a>&nbsp;");
	}	
	...
	for (int i = 1, k = spage ; i <= bsize && k <= epage ; i++, k++) {
		if (cpage == k) {
			...
			out.print("&nbsp;<a href='mp_buyauc.auc?cpage=" + k + args + "'>");
			out.print(k + "</a>&nbsp;");
		}
	}
	...
	if (cpage == pcnt) {
		...
		out.println("&nbsp;<a href='mp_buyauc.auc?cpage=" + (cpage + 1) + args + "'>[&gt;]</a>");
		out.print("&nbsp;<a href='mp_buyauc.auc?cpage=" + pcnt + args + "'>");
		out.println("[&gt;][&gt;]</a>");
	}
