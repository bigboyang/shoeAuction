...
<%@ include file="../../inc/mypage_left.jsp" %>
   ...
   <table id ="table1" class = "table table-bordered table-hover table-condensed" width="800" >
   <tr>
   <th width="15%">기간</th>
   <td colspan="3">
   ...
   <td rowspan="2" width = "5%"><input type="submit" value="검색"  class = "btn btn-default btn-block"/></td>
   </tr>
   <tr>
   ...
   <p style="width:700px;" align="right">정렬조건 :  
         ...
         <option value="mp_waitauc.auc<%=args %>&ord=waitdated" <% if (ord.equals("waitdated")) {%> selected = "selected" <%} %>>최신순</option>
         <option value="mp_waitauc.auc<%=args %>&ord=waitdatea" <% if (ord.equals("waitdatea")) {%> selected = "selected" <%} %>>오래된순</option>
      </select>
   </p>
   <!-- 상품 -->
   ...
   <%
      ...
      out.println("<tr><th colspan='3' text-align='center'>검색 결과가 없습니다.</th></tr>");
   }
   %>
   </table>
   <!-- 상품 리스트 영역 종료 -->
   ...
   <!-- 페이징 영역 시작 -->
      ...
      if (pdtList != null && pdtList.size() > 0) {   
         ...
         if (cpage == 1) {   
            ...
            out.print("<li><a href='mp_waitauc.auc?cpage=1" + args + "'>");
            out.println("[&lt;&lt;]</a></li>");
            out.print("<li><a href='mp_waitauc.auc?cpage=" + (cpage - 1) + args + "'>");
            out.println("[&lt;]</a></li>");
         }   
         ...
         for (int i = 1, k = spage ; i <= bsize && k <= epage ; i++, k++) {
            if (cpage == k) {
               ...
               out.print("<li><a href='mp_waitauc.auc?cpage=" + k + args + "'>");
               out.print(k + "</a></li>");
            }
         }
         ...
         if (cpage == pcnt) {
            ...
            out.println("<li><a href='mp_waitauc.auc?cpage=" + (cpage + 1) + args + "'>[&gt;]</a></li>");
            out.print("<li><a href='mp_waitauc.auc?cpage=" + pcnt + args + "'>");
            out.println("[&gt;][&gt;]</a></li>");
         }
