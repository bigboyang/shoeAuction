...
<div id="outer">
   ...
   <div id="center">
      ...
      <table border="1" width="800" cellpadding="5" class = "table table-bordered table-hover table-condensed">
      <tr>
      <th width="15%">기간</th>
      <td colspan="3" width = "*">
      ...
      <input type="submit" value="검색"  class = "btn btn-default btn-block"/></td>
      </tr>
      <tr>
      <td align="center">
      ...
      <p style="width:700px;" align="right" >정렬조건  
            ...
            <option value="mp_buyauc.etc<%=args %>&ord=startd" <% if (ord.equals("startd")) {%> selected = "selected" <%} %>>최신순</option>
            <option value="mp_buyauc.etc<%=args %>&ord=starta" <% if (ord.equals("starta")) {%> selected = "selected" <%} %>>오래된순</option>
         </select>
      </p>
      <!-- 상품 -->
      <strong style="font-size:110%;"> 총 : <%=pdtList.size() %>개 구매 </strong>
      ...
      if (pdtList != null && pdtList.size() > 0) {
         for (int i = 0 ; i < pdtList.size() ; i++) {
            ...
            String lnk = "<a href='order_view.etc?id=" + pi.getPi_id() + "'>"; %>
      <tr>
      ...
      <% 
      String status = pi.getOi_status();
      String wait = pi.getPw_status();
...
if (pdtList != null && pdtList.size() > 0) {	
	...
	if (cpage == 1) {	
		...
		out.print("<li><a href='mp_buyauc.etc?cpage=1" + args + "'>");
		out.println("[&lt;&lt;]</a></li>");
		out.print("<li><a href='mp_buyauc.etc?cpage=" + (cpage - 1) + args + "'>");
		out.println("[&lt;]</a></li>");
	}	
	...
	for (int i = 1, k = spage ; i <= bsize && k <= epage ; i++, k++) {
		if (cpage == k) {
			...
			out.print("<li><a href='mp_buyauc.etc?cpage=" + k + args + "'>");
			out.print(k + "</a></li>");
		}
	}
	...
	if (cpage == pcnt) {
		...
		out.println("<li><a href='mp_buyauc.etc?cpage=" + (cpage + 1) + args + "'>[&gt;]</a></li>");
		out.print("<li><a href='mp_buyauc.etc?cpage=" + pcnt + args + "'>");
		out.println("[&gt;][&gt;]</a></li>");
	}
