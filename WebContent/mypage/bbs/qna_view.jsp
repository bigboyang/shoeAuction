...
<style>
   ...
   #footer {position:absolute; left:250px; bottom:-350px;}
</style>
</head>
</head>
<body>
...
<h2>1:1 문의</h2>
<form name="frm" action="qna_up_form.etc" method="post">
<input type="hidden" name="bq_cata" value="<%=article.getBq_cata() %>" />
<input type="hidden" name="bq_title" value="<%=article.getBq_title() %>" />
<input type="hidden" name="bq_content" value="<%=article.getBq_content() %>" />
<input type="hidden" name="mi_id" value="<%=article.getMi_id()%>" />
<input type="hidden" name="idx" value="<%=article.getBq_idx()%>" />
...
<tr>
<td>
<% if (article.getBq_img1() == null) {%> 첨부파일이 없습니다. <% } else { %><%=article.getBq_img1() %> <% } %>
<% if (article.getBq_img2() == null) {%> 첨부파일이 없습니다. <% } else { %> <%=article.getBq_img1() %> <% } %>
</td>
</tr>
</table>
<br />
...
<tr>
<td>
<input type="submit" value="문의수정" onclick="sameID()"/>&nbsp;&nbsp;
