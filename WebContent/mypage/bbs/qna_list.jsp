...
String bqcata = pdtpageInfo.getBqcata();
String pkeyword = pdtpageInfo.getPkeyword();
...
<h2>1:1 문의</h2>
<div id="bqcata" style="width:800px;" align="center">
<a href="qna_list.etc">전체</a>&nbsp;&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=구매">구매</a>&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=판매">판매</a>&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=검수">검수</a>&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=배송">배송</a>&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=입찰">입찰</a>&nbsp;&nbsp;
<a href="qna_list.etc?bqcata=기타">기타</a>
</div>
<br />
...
if (qnaList.size() > 0 && rcnt > 0) {
	...
	for (int i = 0; i < qnaList.size(); i++) {
		...
		lnk = "<a href='qna_view.etc?idx=" + qi.getBq_idx() + args + "'>";
%>
<tr align="center">
<td><%=qi.getBq_cata() %></td>
<td><%=qi.getMi_id() %></td>
...
<td><% if(qi.getBq_adate() == null) {%>처리중<% } else { %><%=qi.getBq_adate().substring(2, 11).replace('-', '.')%><% } %></td>
...
<tr>
<td align="right">
<input type="button" value="글 등록" onclick="location.href='qna_in_form.etc'"/>
</td>
</tr>
</table>
 </form>  
